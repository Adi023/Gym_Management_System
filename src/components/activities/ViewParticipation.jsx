...
export default function ViewParticipation() {
  ...
  const fetchData = async () => {
    try {
      const responseData = await ActivityMapperServices.viewActivityMapper(0, 5, "id", "asc");
  ...
  const handlePageChange = async (pageNumber = 0) => {
    try {
      const responseData = await ActivityMapperServices.viewActivityMapper(pageNumber, pageSize, sortBy, sortDir);
      setPostContent(responseData.data);
    } catch (error) {
      console.error('Error fetching data:', error);
    }
