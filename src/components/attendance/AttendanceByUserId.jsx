...
export default function AttendanceByUserId() {
  ...
  const fetchData = async (userId) => {
    try {
      const responseData = await AttendanceServices.viewAttendanceByUserId(0, 5, "attendanceId", "desc",userId);
  ...
  const handlePageChange = async (pageNumber=0) => {
    try {
      const responseData = await AttendanceServices.viewAttendanceByUserId(pageNumber, pageSize, sortBy, sortDir,userId);
      setPostContent(responseData.data);
    } catch (error) {
      console.error('Error fetching data:', error);
    }
